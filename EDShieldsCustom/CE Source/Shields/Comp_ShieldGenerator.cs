...
namespace Jaxxa.EnhancedDevelopment.Shields.Shields
{
    ...
    public class Comp_ShieldGenerator : ThingComp
    {
        ...
        public void UpdateShieldStatus()
        {
            ...
            switch (CurrentStatus)
            {
                ...
                case (EnumShieldStatus.Offline):
                    ...
                    if (_PowerAvalable)
                    {
                        CurrentStatus = EnumShieldStatus.Initializing;
                        m_WarmupTicksRemaining = m_RecoverWarmupDelayTicks;
                    }
                    break;

                case (EnumShieldStatus.Initializing):
                    if (_PowerAvalable)
                    {
        ...
        public bool WillInterceptDropPod(DropPodIncoming dropPodToCheck)
        {
            ...
            //Check if online
            if (CurrentStatus == EnumShieldStatus.Offline || CurrentStatus == EnumShieldStatus.Initializing)
            {
                return false;
            }
        ...
        private bool InterceptCheckBasic(ProjectileProperties projProps, IntVec3 projPos, Thing launcher)
        {
            //Check if online
            if (CurrentStatus == EnumShieldStatus.Offline || CurrentStatus == EnumShieldStatus.Initializing)
            {
            ...
            //Check Distance
            float _Distance = Vector3.Distance(projPos.ToVector3(), parent.Position.ToVector3());
        ...
        public static Boolean CorrectAngleToIntercept(Quaternion projRotation, Vector3 projPosition, Thing shieldBuilding)
        {
            //Detect proper collision using angles
            Quaternion targetAngle = projRotation;

            Vector3 projectilePosition2D = projPosition;
        ...
        public override string CompInspectStringExtra()
        {
            ...
            else if (CurrentStatus == EnumShieldStatus.Initializing)
            {
